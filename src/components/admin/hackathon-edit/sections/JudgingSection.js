...
const JudgingRound1 = dynamic(() => import("../../JudgingRound1"), {
  ssr: false,
});
const JudgingRound2 = dynamic(() => import("../../JudgingRound2"), {
  ssr: false,
});
const JudgingResults = dynamic(() => import("../../JudgingResults"), {
  ssr: false,
});
const PeerVoteResults = dynamic(() => import("../../PeerVoteResults"), {
  ssr: false,
});

const TAB_NAMES = ["round1", "round2", "results", "peer-vote"];

const JudgingSection = ({ admin, accessToken, orgId, onSnack }) => {
  const router = useRouter();
  const eventId = admin.hackathon?.event_id;

  const subtabFromUrl =
    typeof router.query.subtab === "string" ? router.query.subtab : null;
      ...
      <Box sx={{ p: { xs: 2, md: 3 } }}>
        ...
        {activeTab === 3 && (
          <PeerVoteResults
            ...
            orgId={orgId}
            onSnack={onSnack}
            votingCloses={admin.hackathon?.deadlines?.voting_closes}
            enabled={!!admin.hackathon?.constraints?.peer_vote_enabled}
          />
        )}
      </Box>
    </SectionContainer>
