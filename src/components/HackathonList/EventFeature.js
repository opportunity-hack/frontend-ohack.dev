...
const RX = {
  ink: "var(--ink, #16181D)",
  muted: "var(--muted, #5B6270)",
  faint: "var(--faint, #8A8F9A)",
  line: "var(--line, #E7E1D4)",
  brand: "var(--brand, #1B3A6B)",
  accent: "var(--accent, #E2552E)",
  surface: "var(--surface, #FFFFFF)",
  surface2: "var(--surface-2, #F4F1E9)",
...
function EventFeature(props) {
  ...
  const eventLinks =
    typeof rawEventLinks === "string" ? [rawEventLinks] : rawEventLinks;
        ...
        <Link href={`/hack/${event_id}`} passHref>
          <CardContent sx={{ cursor: "pointer", p: 2 }}>
            ...
            >
              <Typography
                variant="h6"
                sx={{ fontWeight: 600, color: "#003486", fontSize: "1.1rem" }}
              >
                {title}
              </Typography>
              {type && (
  ...
  const sameYear =
    validDates && getYear(new Date()) === getYear(parseLocalDate(start_date));
  ...
  return (
    ...
    >
      ...
      <Typography sx={{ mt: 1.5, color: RX.faint, fontSize: "0.85rem" }}>
        ...
        {nonprofits?.length
          ? ` · ${nonprofits.length} nonprofit${nonprofits.length === 1 ? "" : "s"}`
          : ""}
      </Typography>
        ...
        <Box sx={{ mt: 2.5, pt: 2.5, borderTop: `1px solid ${RX.line}` }}>
          ...
          >
            {DONATION_CATEGORIES.map(({ label, key }) => {
              ...
              const pct =
                goal > 0
                  ? Math.min((donationCurrent[key] / goal) * 100, 100)
                  : 0;
              return (
                <Box
                  key={key}
