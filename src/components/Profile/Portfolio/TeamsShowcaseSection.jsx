...
import { WINNING_STATUSES } from "../../../constants/teamStatus";
...
function repoEntries(githubLinks) {
  ...
  for (const item of githubLinks || []) {
    ...
    if (typeof item === "string") {
      entries.push({
        link: item,
        name: item.split("/").slice(-1)[0] || "Repository",
      });
    } else if (item.link) {
      entries.push({
        link: item.link,
        name: item.name || item.link.split("/").slice(-1)[0],
      });
    }
  }
  return entries;
...
// sourced from the single WINNING_STATUSES catalog (constants/teamStatus.js)
// plus the two additional "shipped it" statuses, so this never drifts from
// the canonical list again.
const SHOWCASE_TROPHY_STATUSES = new Set([
  ...WINNING_STATUSES.map((s) => s.value),
  "DEPLOYED",
  "NONPROFIT_SIGNOFF",
]);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      {teams.map((team) => {
        ...
        const isWinner =
          awards.length > 0 || SHOWCASE_TROPHY_STATUSES.has(team.status);
