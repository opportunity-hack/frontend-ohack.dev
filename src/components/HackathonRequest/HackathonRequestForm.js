...
const DISPLAY_FONT = FONT_DISPLAY;
const RX = {
  brand: "#1B3A6B",
  brandInk: "#0E2547",
  accent: "#E2552E",
...
const HackathonRequestForm = ({ initialData, onSubmit, isEdit = false }) => {
  ...
  const handleDateChange = (name, date) => {
    ...
    if (name === "preferredDate" || name === "alternateDate") {
      ...
      setErrors((prev) => ({
        ...prev,
        [name]: isValidCallDate
          ? undefined
          : "Please select a Friday at least 7 days from now and within 30 days",
      }));
    } else if (name === "expectedHackathonDate") {
      const isValidHackathonDate = validateHackathonDate(date);
      setErrors((prev) => ({
        ...prev,
        [name]: isValidHackathonDate
          ? undefined
          : "Hackathon date should be at least 2 months in the future to allow for proper planning",
      }));
    }
  };
  ...
  const validateStep = () => {
    ...
    if (activeStep === 0) {
      ...
      if (!formData.contactEmail) newErrors.contactEmail = "Required";
        ...
        newErrors.contactEmail =
          "Please enter a valid email address (e.g., email@example.com or Name <email@example.com>)";
    ...
    // Step 3 validation
      ...
      const isInternalFocused =
        formData.hackathonTheme === "internal-products" &&
        formData.organizationType === "corporate";

      if (!isInternalFocused) {
        if (formData.hasNonprofitList === null)
          newErrors.hasNonprofitList = "Required";
        if (
          formData.hasNonprofitList !== "yes" &&
          (!formData.nonprofitSource || formData.nonprofitSource.length === 0)
        ) {
          newErrors.nonprofitSource = "Please select at least one option";
        }
        if (!formData.preferredNonprofitLocation)
          newErrors.preferredNonprofitLocation = "Required";
        if (
          formData.preferredNonprofitLocation === "specific-region" &&
          !formData.specificRegion
        ) {
  ...
  const calculatedDonation = Math.round(
    formData.budget * (formData.donationPercentage / 100),
  );
        ...
        {activeStep === 0 && (
          <Box>
            <Typography
              ...
              component="h2"
              gutterBottom
              sx={{ fontFamily: DISPLAY_FONT, fontWeight: 600, color: RX.ink }}
            >
              First, who's hosting?
            </Typography>
            <Typography variant="body1" sx={{ color: RX.muted, mb: 3 }}>
              This shapes the rest of the form so we only ask what's relevant to
              you.
            </Typography>
                ...
                {ORG_TYPES.map((opt) => {
                  ...
                  return (
                    <Grid size={{ xs: 12, sm: 4 }} key={opt.value}>
                      ...
                      >
                        <Icon
                          sx={{ color: selected ? RX.brand : RX.muted, mb: 1 }}
                        />
                        <Typography
                          variant="subtitle1"
                          sx={{
            ...
            <Grid container spacing={3}>
              ...
              <Grid size={{ xs: 12 }}>
                ...
                >
                  <FormLabel component="legend">
                    Who will participate in your hackathon?
                  </FormLabel>
                  <Typography
                    variant="body1"
                    color="textSecondary"
                  ...
                  <FormGroup row>
                    ...
                    {formData.organizationType === "corporate" && (
                      <FormControlLabel
                        control={
                          <Checkbox
                            checked={formData.participantType.includes(
                              "internal-staff",
                            )}
                            onChange={() =>
                  ...
                  {formData.eventFormat === "hybrid" && (
                    <Alert severity="warning" sx={{ mt: 2 }}>
                      <Typography variant="body2" sx={{ fontWeight: 500 }}>
                        Hybrid events require additional coordination and
                        resources. We recommend choosing either in-person or
                        virtual for optimal engagement.
                      </Typography>
                    </Alert>
                  )}
                </FormControl>
              ...
              <Grid size={{ xs: 12 }}>
                <FormControl fullWidth component="fieldset" margin="normal">
                  <FormLabel component="legend">
                    What theme or focus would you like for your hackathon?
                  </FormLabel>
                  <RadioGroup
                    row
                    name="hackathonTheme"
              ...
              <Grid size={{ xs: 12 }}>
                <Typography variant="h6" sx={{ mt: 3, mb: 2 }}>
                  When would you like to host your hackathon?
                </Typography>
                <Alert severity="info" sx={{ mb: 3 }}>
                  <Typography variant="body1" sx={{ fontWeight: 500 }}>
                    💡 We recommend planning at least 2 months in advance to
                    ensure a successful event with proper preparation.
                  </Typography>
                </Alert>
                ...
                <LocalizationProvider dateAdapter={AdapterDateFns}>
                  <DatePicker
                    ...
                    onChange={(date) =>
                      handleDateChange("expectedHackathonDate", date)
                    }
                    slotProps={{
                      textField: {
                        fullWidth: true,
                        ...
                        helperText:
                          errors.expectedHackathonDate ||
                          "Plan for at least 2 months of preparation time",
                      },
                    }}
              ...
              <Grid size={{ xs: 12 }}>
                ...
                >
                  <Typography
                    variant="body1"
                    sx={{ fontWeight: 500, color: RX.ink }}
                  >
                    📞 Pick a Friday that works for an initial planning call.
                    We'll confirm the details after reviewing your request — and
                    if none of these dates work, we'll find another time
                    together.
                  </Typography>
                </Box>
              </Grid>
        ...
        {activeStep === 2 && (
          <Box>
            <Typography
              ...
              sx={{ fontFamily: DISPLAY_FONT, fontWeight: 600, color: RX.ink }}
            >
              {["internal-products", "api-integration"].includes(
                formData.hackathonTheme,
              ) && formData.organizationType === "corporate"
                ? "Internal Focus & Engagement"
                : formData.hackathonTheme === "educational" &&
                    formData.organizationType === "university"
              ...
              formData.organizationType === "corporate" && (
                ...
                >
                  <Box sx={{ display: "flex", alignItems: "flex-start" }}>
                    <InfoIcon
                      sx={{ color: theme.palette.info.main, mr: 2, mt: 0.5 }}
                    />
                    <Typography variant="body1">
                      You've selected an internal product-focused hackathon.
                      While this won't directly involve nonprofits, we can still
                      help you organize an effective event that drives
            ...
            {formData.hackathonTheme === "api-integration" &&
              formData.organizationType === "corporate" && (
                <Paper
                  elevation={0}
                  sx={{
                ...
                >
                  <Box sx={{ display: "flex", alignItems: "flex-start" }}>
                    ...
                    />
                    <Typography variant="body1">
                      You've selected an API/Product integration hackathon. You
                      can choose to work with nonprofits who might benefit from
                      your products, or focus on developers building with your
                      APIs.
                    </Typography>
                  </Box>
                </Paper>
              ...
              formData.organizationType === "university" && (
                ...
                >
                  <Box sx={{ display: "flex", alignItems: "flex-start" }}>
                    ...
                    />
                    <Typography variant="body1">
                      You've selected an educational hackathon. This can be
                      aligned with curriculum goals while still supporting
                      nonprofits, or can focus entirely on educational outcomes.
                    </Typography>
                  </Box>
                </Paper>
              )}
            ...
            {!(
              ["internal-products"].includes(formData.hackathonTheme) &&
              formData.organizationType === "corporate"
            ) ? (
              <>
                <Box sx={{ mb: 4 }}>
                  <FormControl fullWidth component="fieldset" margin="normal">
                    <FormLabel component="legend">
                      Do you already have nonprofits/NGOs identified for this
                      hackathon?
                    </FormLabel>
                    <RadioGroup
                      row
                    ...
                    >
                      <FormControlLabel
                        value="yes"
                        control={<Radio />}
                        label="Yes, we have a complete list"
                      />
                      <FormControlLabel
                        value="partial"
                        control={<Radio />}
                        label="We have some identified"
                      />
                      <FormControlLabel
                        value="no"
                        control={<Radio />}
                        label="No, we need help"
                      />
                    </RadioGroup>
                  ...
                  {(formData.hasNonprofitList === "yes" ||
                    ...
                    <>
                      ...
                      >
                        <FormLabel component="legend">
                          Have you worked with these nonprofits/NGOs before?
                        </FormLabel>
                        <RadioGroup
                          row
                        ...
                        >
                          ...
                          <FormControlLabel
                            value="no"
                            control={<Radio />}
                            label="No, this would be our first collaboration"
                          />
                        </RadioGroup>
                      </FormControl>
                    </>
                      ...
                      <FormGroup>
                        {formData.hasNonprofitList !== "partial" && (
                          <FormControlLabel
                            control={
                              <Checkbox
                                checked={formData.nonprofitSource.includes(
                                  "own-list",
                                )}
                                onChange={() =>
                                  handleMultiSelectChange({
                                    target: {
                                      name: "nonprofitSource",
                                      value: "own-list",
                                    },
                                  })
                                }
                ...
                <Box>
                  <FormControl fullWidth component="fieldset" margin="normal">
                    ...
                    >
                      <FormControlLabel
                        value="local"
                        control={<Radio />}
                        label="Local to our event location"
                      />
                      <FormControlLabel
                        value="global"
                        control={<Radio />}
                        label="Global nonprofits welcome"
                      />
                      <FormControlLabel
                        value="specific-region"
                        control={<Radio />}
                        label="From a specific region"
                      />
                    </RadioGroup>
                  ...
                  {formData.preferredNonprofitLocation ===
                    "specific-region" && (
                    <TextField
                      fullWidth
                      margin="normal"
                      ...
                      helperText={
                        errors.specificRegion ||
                        "E.g., Southeast Asia, East Africa, European Union, etc."
                      }
                      required
                    />
                  )}
        ...
        {activeStep === 3 && (
          <Box>
            ...
            <Typography
              variant="body1"
              paragraph
              sx={{ mb: 3, color: RX.muted }}
            >
              A rough sense of who'll handle each piece helps us know where to
              lean in. Not sure yet? Leave the suggested defaults — nothing here
              is final, and we'll sort it all out together on the call.
            </Typography>
              ...
              <Grid size={{ xs: 12, md: 6 }}>
                ...
                >
                  <Typography
                    ...
                    component="h3"
                    gutterBottom
                    sx={{ fontWeight: 600, color: RX.brand }}
                  >
                    Event Logistics
                  </Typography>
                  ...
                  <Box sx={{ mb: 2 }}>
                    ...
                    <RadioGroup
                      ...
                      onChange={(e) =>
                        handleResponsibilityChange("venue", e.target.value)
                      }
                    >
                      <FormControlLabel
                        value="requestor"
                  ...
                  <Box sx={{ mb: 2 }}>
                    ...
                    <RadioGroup
                      ...
                      onChange={(e) =>
                        handleResponsibilityChange("food", e.target.value)
                      }
                    >
                      <FormControlLabel
                        value="requestor"
                  ...
                  <Box>
                    ...
                    <RadioGroup
                      ...
                      onChange={(e) =>
                        handleResponsibilityChange("prizes", e.target.value)
                      }
                    >
                      <FormControlLabel
                        value="requestor"
                ...
                >
                  ...
                  <Box sx={{ mb: 2 }}>
                    ...
                    <RadioGroup
                      ...
                      onChange={(e) =>
                        handleResponsibilityChange("judges", e.target.value)
                      }
                    >
                      <FormControlLabel
                        value="requestor"
                  ...
                  <Box sx={{ mb: 2 }}>
                    ...
                    <RadioGroup
                      ...
                      onChange={(e) =>
                        handleResponsibilityChange("mentors", e.target.value)
                      }
                    >
                      <FormControlLabel
                        value="requestor"
                  ...
                  <Box>
                    ...
                    <RadioGroup
                      ...
                      onChange={(e) =>
                        handleResponsibilityChange("marketing", e.target.value)
                      }
                    >
                      <FormControlLabel
                        value="requestor"
                  ...
                  <Grid container spacing={2}>
                    <Grid size={{ xs: 12, md: 6 }}>
                      <Box sx={{ mb: { xs: 2, md: 0 } }}>
                        ...
                        >
                          Nonprofit Recruitment & Onboarding
                        </Typography>
                        <RadioGroup
                          row
                    ...
                    <Grid size={{ xs: 12, md: 6 }}>
                      <Box>
                        ...
                        <RadioGroup
                          ...
                          value={
                            formData.responsibilities.participantRecruitment
                          }
                          onChange={(e) =>
                            handleResponsibilityChange(
            ...
            <Alert severity="info" sx={{ mt: 2 }}>
              <Typography variant="body1" sx={{ fontWeight: 500 }}>
                💡 Note: Final responsibilities will be agreed upon during our
          ...
          <Box>
            ...
            <Box sx={{ mb: 4 }}>
              ...
              >
                {isCorporate
                  ...
                  : "Do you have any funding? (optional)"}
                <Tooltip
                  title={
              ...
              <Typography
                ...
                sx={{ fontWeight: 600 }}
              >
                {isCorporate ? "Suggested budget" : "Funding you have in mind"}:
              ...
              >
                ...
                <Typography variant="body2" sx={{ mb: 2, color: RX.muted }}>
                  You don't need any of this lined up to apply. It's just so you
                  know what's involved.
                </Typography>
                <Box component="ul" sx={{ pl: 2, m: 0 }}>
                  <Box component="li" sx={{ mb: 1 }}>
                    <Typography variant="body1" sx={{ fontWeight: 500 }}>
                      $97 per participant - Meals and refreshments (36 hours of
                      continuous hacking)
                    </Typography>
                  </Box>
                  <Box component="li" sx={{ mb: 1 }}>
                ...
                <Alert severity="info" sx={{ mt: 2 }}>
                  <Typography variant="body1" sx={{ fontWeight: 500 }}>
                    ℹ️ Note: Event venue costs are not included and are
                    typically provided by the host organization.
                  </Typography>
                </Alert>
              </Paper>
            </Box>
                ...
                <Grid size={{ xs: 12, md: 6 }}>
                  <Box sx={{ display: "flex", mb: 1 }}>
                    ...
                    <Typography variant="body2">
                      Event coordination and a planning playbook
                    </Typography>
                  </Box>
                  <Box sx={{ display: "flex", mb: 1 }}>
                    <CheckCircleIcon
                    ...
                    />
                    <Typography variant="body2">
                      Mentors and judges from our community
                    </Typography>
                  </Box>
                </Grid>
                <Grid size={{ xs: 12, md: 6 }}>
                  <Box sx={{ display: "flex", mb: 1 }}>
                    ...
                    />
                    <Typography variant="body2">
                      Marketing support for your event
                    </Typography>
                  </Box>
                  <Box sx={{ display: "flex", mb: 1 }}>
                    <CheckCircleIcon
                    ...
                    />
                    <Typography variant="body2">
                      Access to our vetted nonprofit network
                    </Typography>
                  </Box>
                </Grid>
              </Grid>
            ...
            {isCorporate && (
              ...
              >
                ...
                >
                  <FavoriteIcon
                    sx={{ mr: 1, verticalAlign: "bottom", color: RX.accent }}
                  />
                ...
                <Typography variant="body1" sx={{ color: RX.muted, mb: 2 }}>
                  ...
                  every event. It funds technical support for nonprofits,
                  project hosting, and educational resources for volunteers.
                </Typography>

                <Box sx={{ mb: 1 }}>
                  <Typography gutterBottom>
                    Suggested contribution:{" "}
                    <strong>{formData.donationPercentage}%</strong> of your
                    event budget (${calculatedDonation.toLocaleString()})
                  </Typography>
                  <Slider
            ...
            >
              ...
              <Box sx={{ mb: 2 }}>
                <FormControlLabel
                  ...
                  label={
                    <Typography variant="body1" sx={{ fontWeight: 500 }}>
                      I agree to be contacted by the Opportunity Hack team to
                      discuss my request
                    </Typography>
                  }
                />
              ...
              <Box>
                <FormControlLabel
                  ...
                  label={
                    <Typography variant="body1" sx={{ fontWeight: 500 }}>
                      I understand that hackathons require at least 2 months of
                      preparation time for optimal success
                    </Typography>
                  }
                />
