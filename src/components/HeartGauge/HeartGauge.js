...
const HeartGauge = ({ history }) => {
  ...
  const [viewMode, setViewMode] = useState("milestone"); // 'milestone' or 'gauge'
  ...
  const nextReward =
    rewards.find((r) => r.hearts > hearts) || rewards[rewards.length - 1];

  // New milestone-focused view
  if (viewMode === "milestone") {
    return (
      ...
      >
        <Box sx={{ mb: 2, alignSelf: "flex-end" }}>
          <ButtonGroup size="small" variant="outlined">
            <Button
              ...
              variant={viewMode === "milestone" ? "contained" : "outlined"}
              startIcon={<FaListUl />}
            >
              Milestones
            </Button>
            <Button
              ...
              variant={viewMode === "gauge" ? "contained" : "outlined"}
        ...
        <MilestoneProgress history={history} />
      </Box>
    );
  }
    ...
    >
      ...
      >
        ...
        >
          ...
          <Typography variant="body1">{nextReward.reward}</Typography>
