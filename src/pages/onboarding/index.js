...
const onboardingTheme = createTheme({
  ...
  components: {
    ...
    MuiCard: {
      styleOverrides: {
        root: { boxShadow: "none", border: "1px solid #E7E1D4" },
      },
    },
...
function OnboardingComponent() {
  ...
  const [openCongratulatoryDialog, setOpenCongratulatoryDialog] =
    useState(false);
  ...
  useEffect(() => {
    ...
    if (!cookies[ONBOARDING_VISITED_COOKIE]) {
      setCookie(ONBOARDING_VISITED_COOKIE, "true", {
        path: "/",
        maxAge: 31536000,
      });
  ...
  useEffect(() => {
    ...
    if (savedProgress) {
      try {
        ...
        const savedHighestStep = localStorage.getItem(
          "ohack_onboarding_highest_step",
        );
        if (savedHighestStep) {
          try {
  ...
  const handleNext = () => {
    if (activeStep === steps.length - 1) {
      ...
      trackEvent({
        ...
        params: {
          event_label: steps[activeStep + 1]?.label,
          step: activeStep + 2,
          page: "onboarding",
        },
      });
  ...
  const handleComplete = async () => {
    ...
    try {
      ...
      trackEvent({
        ...
        params: { event_label: "all_steps", page: "onboarding" },
      });
      setOpenCongratulatoryDialog(true); // Open the dialog
    } catch (err) {
      console.error("Error completing onboarding:", err);
      setError(
        "There was an error completing the onboarding process. Please try again.",
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <>
        ...
        >
          ...
          >
            <Stepper
              ...
              alternativeLabel
              sx={{ ...stepperSx, minWidth: { xs: 720, md: 0 } }}
            >
          ...
          >
            ...
            {activeStep === steps.length - 1 ? (
              ...
              >
                Complete onboarding{" "}
                {loading ? (
                  <CircularProgress size={18} sx={{ color: "#fff", ml: 0.5 }} />
                ) : (
                  <Arrow />
                )}
              </button>
      ...
      >
        ...
        <DialogContent>
          <DialogContentText
            id="congratulations-dialog-description"
            sx={{ fontSize: "1.05rem" }}
          >
