...
- **Backend gap (open):** `backend-ohack.dev/services/volunteers_service.py` accepts and stores those three fields on the volunteer doc but **nothing reads them downstream** — no submit-side validation that the deposit was actually paid, no admin view of paid-vs-unpaid hackers, no refund job after the event. This works today only because `hacker_deposit.enabled` is false for current events. Before any event sets `hacker_deposit.enabled = true` in `constraints`, add (1) a backend check that rejects submission when deposit is required and `stripe_payment_intent_id` is missing, (2) a Stripe webhook (`checkout.session.completed`) so the source of truth isn't a self-reported frontend field, (3) an admin list view, and (4) a refund-after-event task that reads `deposit_disposition` and either refunds or marks as donated.
