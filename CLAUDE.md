...
**Rank semantics (`/projects`): rank 1 is the BEST, ascending, unranked last.** `rankValue()` in `ProjectList.js` normalizes (`Number`, `>0`, else `Infinity`). Featured strip = every non-paused project with rank 1 (plus the legacy `featured` flag, which is set on zero DB rows), ordered fewest-helpers-first; fallback when nothing is rank 1 = best-ranked non-production/non-post-hackathon/non-paused, top 3. "Most relevant" sorts rank ascending (don't flip it back to descending — that floated rank-99 filler to the top for years). "Needs help first" **excludes** non-recruiting projects (production, paused — the same `acceptsNewHelpers()` rule as the help toggle) unless the status chip explicitly selects that status.
