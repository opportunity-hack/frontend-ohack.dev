...
- **`VolunteerTable`**: judges-only `training` + `introVideo` columns (`sortable: false`, honored in the header) via module-scope `trainingChipConfig` (also used by the mobile card view). Chip falls back to `volunteer.judgeTrainingCompleted` while LMS data is pending/unavailable.
...
## Printable event welcome guide (`/hack/[event_id]/print`) + admin CSV/print (Sep 2026, #229)

`src/pages/hack/[event_id]/print.js` is a coordinator-facing handout: cover (title / dates / location / description + `QRCodeSVG` → `https://www.ohack.dev/hack/<event_id>`), then optional sections Schedule / Mentors / Judges / Volunteers, each starting on a new printed page. Selection lives in `?sections=schedule,mentors` (omitted = all; `parseSections`/`serializeSections`) synced via shallow `router.replace`. Data is `getServerSideProps`: the event doc + the three PUBLIC `/api/messages/hackathon/<id>/{mentor,judge,volunteer}` lists filtered to `isSelected` and projected through `pickPerson` (only rendered fields reach the page; `s-maxage=60`). Pure helpers + unit tests live in `src/components/Hackathon/printGuideData.js` (+ `__tests__`). "Event days only" filters countdowns to a **padded** window (start−2d … end+1d, `EVENT_WINDOW_PAD_*`) because event docs are sometimes a day off (2025_fall's `start_date` is the Sunday of a Sat–Sun event); it falls back to all items if nothing matches. `_app.js` hides NavBar/Footer for this route (`isPrintPage`); the page has its own "Back to event" link and is `noindex`. Entry point: the "Print guide" button beside "Agenda" in `EventCountdown`. The old `/hack/[event_id]/print-timeline` page (auto-print + hardcoded placeholder WiFi/parking/emergency text) was deleted — `next.config.js` 307s it to `/print?sections=schedule`. Hackers are deliberately not a section (100+ people, no value on a handout). Anything rendered from `new Date()` on this page must be client-only (the footer "Printed on" date uses an effect) — SSR/CSR date drift is a hydration error.

**Admin volunteer table CSV + Print (`src/components/admin/VolunteerTable.js`):** CSV exports every column of the active tab through `getPlainCellValue` — keep it in sync when adding columns (`certificates`/`training`/`introVideo`/`deposit` have explicit cases; `getDepositLabel` is exported from `HackerDepositChip.js`); UTF-8 BOM; filename `<event_id>-<type>.csv`. Print does NOT call `window.print()`: the workbench sits inside `HackathonAdminLayout`'s `overflowY: auto` content box, which prints a single clipped viewport. Instead `buildPrintDocument` renders a standalone landscape HTML table (`PRINT_HIDDEN_COLUMNS` drops audit-only columns) into a new tab and prints there. Both helpers are module-scope.
