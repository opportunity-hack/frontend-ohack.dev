...
- **Still open:** no Stripe webhook (the frontend's session-status read is the source of truth for the initial payment, which is fragile if a user bails after Stripe charges them but before they return to the success URL). No bulk-refund admin action. Refund status field is set synchronously by the API response, so a Stripe refund that succeeds async (rare for <$50) would briefly show "refunded" before settling — fine in practice.
