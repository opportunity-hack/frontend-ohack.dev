...
**Verification safety (mandatory):** the developer backend on `localhost:6060` is bound to the **production** Firestore (`ohack-dev`) and real Slack/Resend credentials. Treat it as **read-only** during this program: never call `POST /project`, `/project/submit`, `/mentor-availability`, `/peer-vote/*`, `/deadlines/remind`, `PATCH /api/messages/hackathon`, or admin publish/void against :6060. Exercise write paths through (a) pytest with MockFirestore and (b) the **sandbox backend on `localhost:6061`** started by the orchestrator from the backend branch with `FIREBASE_CERT_CONFIG` = the `test-ohack-dev` credential and all Slack/Resend/OpenAI tokens blanked (outbound calls fail harmlessly). For UI checks that need writes, run the frontend with `NEXT_PUBLIC_API_SERVER_URL=http://localhost:6061 npm run dev`. Never post to real Slack channels from a test.
