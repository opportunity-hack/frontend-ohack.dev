...
const integ = await agent(
  `Execute WS-F of the plan in ${FRONT}: merge branches ${done.map((f) => f.res.branch).join(', ')} into feat/team-dashboard-devpost-replacement (resolve conflicts preferring the plan's contracts), run npm run build, eslint + prettier on all touched files, run the Part 6 checklist honoring its Verification safety rule against the test-Firestore backend on :6060 (report each item pass/fail honestly), update CLAUDE.md, docs/refined-design-system.md and ga-events-reference.md as WS-F specifies, and commit (no push). ${COMMON}`,
